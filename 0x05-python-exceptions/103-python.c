...
/**
* print_python_bytes - Confirms if the item is a Python byte assign.
* @p: Pointer to a Python object.
* Return: void.
*/
void print_python_bytes(PyObject *p)
...
/**
* print_python_float - Confirms if the item is a Python float object.
* @p: Pointer to a Python object.
* Return: void.
*/
void print_python_float(PyObject *p)
{
	...
	if (PyFloat_Check(p))
	{
		mode = PyObject_Repr(p);
		mode = PyUnicode_AsEncodedString(mode, "utf-8", "~E~");
		printf("  value: %s\n", ((PyBytesObject *) mode)->ob_sval);
	}
	else
	{
